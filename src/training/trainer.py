...
class Trainer:
    ...
    def __init__(self, config_path: str = "configs/config.yaml"):
        ...
        # 증강 설정 로드 (config.yaml에 따라)
        self.augmentation_enabled = self.data_cfg.get("augmentation_enabled", False)
        self.augmentation_config = {}
        if self.augmentation_enabled:
            augmentation_config_path = self.data_cfg.get("augmentation_config")
            if augmentation_config_path:
                self.augmentation_config = load_config(augmentation_config_path)
                self.logger.info(f"Augmentation enabled, config loaded from: {augmentation_config_path}")
            else:
                self.logger.warning("Augmentation enabled but no augmentation_config path specified. Using default (empty) augmentation config.")
    ...
    def setup_data(self):
        ...
        # 학습 데이터 증강 설정
        if self.augmentation_enabled:
            train_transform = get_train_augmentation(self.augmentation_config, img_size)
            self.logger.info("Training data augmentation is ENABLED.")
        else:
            train_transform = get_transforms(img_size)
            self.logger.info("Training data augmentation is DISABLED.")

        # 검증 데이터 변환은 항상 기본 get_transforms 사용 (증강 없음)
